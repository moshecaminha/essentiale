    ...
    <div className="card stack checkout-sum">
      ...
      {cart.map((i) => (
        <div className="sum-row" key={i.id}>
          <span>{i.qty}x {i.n}{i.deal ? " (-5%)" : ""}</span>
          <span>{brl(unit(i) * i.qty)}</span>
        </div>
      ))}
          ...
          {suggestions.map((s) => (
            <div className="xsell-item" key={s.id}>
              ...
              <div className="xsell-info">
                ...
                <span className="xsell-price"><s>{brl(s.p)}</s> por <strong>{brl(Math.round(s.p * 0.95))}</strong></span>
              </div>
      ...
      <a className="checkout-wa-link" href={`https://wa.me/5581999089912?text=${encodeURIComponent("Olá! Quero finalizar meu pedido na Essentiale.")}`} target="_blank" rel="noopener noreferrer">
        <MessageCircle size={13} /> Finalizar ou tirar dúvida no WhatsApp
      </a>
    </div>
  );
            ...
            <div className="stack">
              {user === undefined ? (
                ...
                <div className="card stack">
                  ...
                  <p className="cell-muted">Crie sua conta em segundos ou entre para acompanhar seu pedido, ver o histórico e receber atualizações. Você pode revisar o carrinho e continuar comprando ao lado.</p>
